...
const api = {
	...
	context: async (
		...
		const url = new URL(
			`translation/${sourceLanguage}-${targetLanguage}/${encodeURIComponent(text).replace(
				/%20/g,
				'+'
			)}`,
		...
		const sourceExamples =
			document
				.querySelector(`.example > div.src.${sourceDirection} > span.text`)
				?.textContent?.trim()
				.split('\n') || [];

		const targetExamples =
			document
				.querySelector(`.example > div.trg.${targetDirection} > span.text`)
				?.textContent?.trim()
				.split('\n') || [];
